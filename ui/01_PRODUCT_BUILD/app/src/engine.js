(function(){
  ...
  async function refreshJournal(filters={}){const st=S.get();if(!st.caseId)return null;const query={since:filters.since!==undefined?filters.since:st.journalSince,as_of_date:filters.as_of_date!==undefined?filters.as_of_date:st.journalAsOf,workstream:filters.workstream!==undefined?filters.workstream:st.journalWorkstream,kind:filters.kind!==undefined?filters.kind:st.journalKind,close_state_id:filters.close_state_id!==undefined?filters.close_state_id:st.journalCloseState,...filters};try{const journal=await API.adapter().loadJournal(query);S.set({journal,journalSince:query.since||'',journalAsOf:query.as_of_date||'',journalWorkstream:query.workstream||'',journalKind:query.kind||'',journalCloseState:query.close_state_id||''});return journal}catch(e){S.set({lastError:errorShape(e)});throw e}}
  ...
  async function openCase(caseId){if(!caseId)return;if(caseId!==S.get().caseId){const q=new URLSearchParams(location.search);q.set('mode',modeQuery());q.set('case',caseId);q.delete('session');location.search='?'+q.toString();return}S.set({scale:'deal',view:'deal-command'});syncRoute()}
  function setView(view){const room=K.rooms.find(r=>r.id===view);if(room?.gated){const gate=view==='decision'?decisionGate():executionGate();if(!gate.ok){S.set({view:'blocked',lastError:{code:gate.code,message:gate.message,details:gate.details||{}}});syncRoute();return}}S.set({view,scale:view==='fund-command'?'fund':'deal',lastError:null});syncRoute();if(view==='journal')refreshJournal().catch(e=>toast(`Journal unavailable: ${e.message}`,'error'))}
  ...
  async function settleCurrent(extra={}){
    ...
    try{
      ...
      const nextProjection=out.projection?.projection||out.projection||st.projection;S.set({busy:false,settlement:out,registry:out.registry||st.registry,context:out.context||st.context,projection:nextProjection,view:'settled'});await refreshJournal().catch(()=>null);status('Canonical Current state returned by the server');syncRoute();
