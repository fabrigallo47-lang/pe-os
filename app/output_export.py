...
def render_export(saved, fmt, origin):
    ...
    profile = saved.get('editorialProfile')
    editorial_note = ''
    if profile:
        fund = (profile.get('fund') or {}).get('name', 'Unbound default')
        editorial_note = f'<p>Editorial profile: {escape(profile["config"]["name"])} · {escape(fund)} · version {profile["version"]}</p><details><summary>Frozen editorial brief</summary><pre>{escape(json.dumps(profile, ensure_ascii=False, indent=2))}</pre></details>'
