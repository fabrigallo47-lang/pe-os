...
import { unknownById, workItemById, actorById, caseReadingById, questionById, workstreamById, workItemDisplayState } from '../app/selectors';

export function Resolve(){
  const {snapshot,focusedQuestionId,focusedWorkstreamId,execute,actor,setActiveObject}=usePanta();
  ...
  return <main className="p-page p-resolve-page">
    <section className="p-resolve-head"><div><div className="p-kicker">Resolve</div><h1>Close the evidence gap</h1><p>{reading.text}</p></div><div className="p-resolve-summary"><strong>{routes.length} evidence routes</strong><span>{routes.filter(m=>m?.status==='ACTIVE'||m?.status==='BLOCKED').length} active · {routes.filter(m=>m&&!m.ownerActorId&&m.status!=='COMPLETED'&&m.status!=='CANCELLED'&&m.kind!=='PANTA_PROPOSAL').length} unassigned · {routes.filter(m=>m?.kind==='PANTA_PROPOSAL'&&m?.status==='PROPOSED').length} PANTA proposal</span></div></section>
    ...
    <section className="p-mission-field">
      <div className="p-mission-routes"><div className="p-section-heading"><strong>Evidence routes</strong><span>{routes.length}</span></div>{routes.map((m,i)=>{if(!m)return null;const owner=actorById(snapshot,m.ownerActorId);return <button key={m.id} className={`p-mission-route ${selectedId===m.id?'is-selected':''}`} onClick={()=>setSelectedId(m.id)}><span className="p-route-index">{String(i+1).padStart(2,'0')}</span><div><strong>{m.name}</strong><small>{owner?.displayName??workItemDisplayState(m)}</small></div><p>{m.whatToObtain??'Claim route'}</p><b>→</b></button>})}</div>

      {selected&&<aside className="p-mission-aperture"><div className="p-kicker">Selected route</div><h2>{selected.name}</h2>{selected.kind==='PANTA_PROPOSAL'&&<p className="p-proposal-state">PANTA proposal · not adopted</p>}<Detail title="What to obtain">{editing?<textarea className="p-input" value={proposalText || selected.whatToObtain || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>)=>setProposalText(e.target.value)}/>:selected.whatToObtain??'Evidence to obtain is not yet mapped.'}</Detail><Detail title="What this can change">{selected.canChangeObjectIds.length?selected.canChangeObjectIds.map(id=><button key={id} className="p-related-link" onClick={()=>void setActiveObject(id)}>{label(snapshot,id)}</button>):'No mapped case object.'}</Detail><Detail title="What would still remain open">{selected.remainingUnknownIds.length?selected.remainingUnknownIds.map(id=><button key={id} className="p-related-link" onClick={()=>void setActiveObject(id)}>{unknownById(snapshot,id)?.title??id}</button>):'No additional mapped gap.'}</Detail><div className="p-human-boundary"><strong>Requires human authority</strong><p>Adopting the route does not authorize external contact or spend.</p></div><div className="p-action-row">{selected.kind==='PANTA_PROPOSAL'&&selected.status==='PROPOSED'&&<>{editing?<><button className="p-btn p-btn-primary" onClick={()=>{void execute({type:'UPDATE_WORK_ITEM_PROPOSAL',workItemId:selected.id,whatToObtain:proposalText || selected.whatToObtain || ''});setEditing(false)}}>Save proposal</button><button className="p-btn" onClick={()=>setEditing(false)}>Cancel</button></>:<><button className="p-btn p-btn-primary" disabled={!canAdopt} onClick={()=>void execute({type:'ADOPT_WORK_ITEM',workItemId:selected.id})}>Adopt this route</button><button className="p-btn" onClick={()=>{setProposalText(selected.whatToObtain||'');setEditing(true)}}>Edit</button><button className="p-btn p-btn-quiet" onClick={()=>void execute({type:'DISMISS_WORK_ITEM_PROPOSAL',workItemId:selected.id})}>Dismiss</button></>}</>}{!selected.ownerActorId&&selected.kind!=='PANTA_PROPOSAL'&&selected.status!=='COMPLETED'&&selected.status!=='CANCELLED'&&<button className="p-btn p-btn-primary" disabled={!canAssign||!snapshot.actors[0]} onClick={()=>snapshot.actors[0]&&void execute({type:'ASSIGN_WORK_ITEM',workItemId:selected.id,ownerActorId:snapshot.actors[0].id})}>Assign owner</button>}</div></aside>}
    </section>
  </main>;
}
