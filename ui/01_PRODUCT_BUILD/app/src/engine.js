(function(){
  ...
  let transitionTimer=null,ingestTimer=null,requestEpoch=0;
  ...
  function startReview(){const ev=activeEvent();if(!ev){toast('No pending event is available.','warning');return}S.set({activeEventId:ev.event_id,view:'change-review',selectedChangeIds:[],activeRun:null,transition:null,authorityRecord:null,executionPackage:null,settlement:null,editingTreatment:false,treatmentDraft:ev.proposed_treatment?.statement||ev.proposed_treatment||''});syncRoute()}
  ...
  function selectAllChanges(){S.set({selectedChangeIds:(S.get().transition?.artifact_change_sets||[]).map(x=>x.artifact_id||x.change_id||x.id)})}
  function decisionGate(){const st=S.get(),t=st.transition,hs=t?.human_stops?.find(x=>x.status==='OPEN')||t?.human_stops?.[0];if(!st.activeRun?.run_id)return{ok:false,code:'RUN_REQUIRED',message:'A valid transition run is required.'};if(!t?.candidate_state_id)return{ok:false,code:'CANDIDATE_REQUIRED',message:'A current Candidate state is required.'};if(!hs)return{ok:false,code:'HUMAN_STOP_REQUIRED',message:'This run does not contain an open Human Stop.'};return{ok:true,human_stop:hs}}
  ...
  async function prepareResponse(){if(!S.get().selectedChangeIds.length){toast('Select at least one change. There is no hidden select-all.','warning');return}try{const out=await API.adapter().prepareRun(S.get().activeRun.run_id,S.get().selectedChangeIds);if(out.run)S.set({activeRun:out.run});const gate=decisionGate();if(gate.ok)setView('decision');else await settleCurrent()}catch(e){toast(e.message,'error')}}
  function selectCourse(id){S.set({selectedCourseId:id})}
  async function attestDecision(){const st=S.get(),gate=decisionGate();if(!gate.ok){setView('decision');return}if(st.mobileReadOnly){toast('Formal attestation is desktop-only.','warning');return}if(!st.selectedCourseId){toast('Select exactly one course of action.','warning');return}const required=gate.human_stop.required_authority_level||'DEAL_PARTNER';if(!hasAuthority(required)){S.set({view:'blocked',lastError:{code:'INSUFFICIENT_AUTHORITY',message:`${required} is required; viewer projection does not grant authority.`}});return}try{S.set({busy:true,operation:'ATTEST'});const out=await API.adapter().attest(st.activeRun.run_id,{run_id:st.activeRun.run_id,candidate_state_id:st.transition.candidate_state_id,human_stop_id:gate.human_stop.stop_id,course_id:st.selectedCourseId,actor_id:actor().actor_id,actor_role:actor().role,artifact_hash:st.transition.replay_hash,idempotency_key:`AUTH-${st.activeRun.run_id}-${st.selectedCourseId}`});const ar=out.authority_record;C.validateAuthorityRecord(ar);S.set({busy:false,authorityRecord:ar,executionPackage:out.execution_package||null,registry:out.registry||st.registry});if(ar.effect_type==='DEFER'||!out.execution_package){await settleCurrent({deferred:true});return}setView('execution')}catch(e){S.set({busy:false});toast(`Attestation rejected: ${e.message}`,'error')}}
  async function sendExecution(simulateFailure=false){const gate=executionGate();if(!gate.ok){setView('execution');return}try{S.set({busy:true,operation:'DELIVERY',executionPackage:{...S.get().executionPackage,status:'SENDING'}});const out=await API.adapter().sendPackage(S.get().executionPackage.execution_package_id,simulateFailure);const pkg=out.execution_package||out;S.set({busy:false,executionPackage:pkg,executionOutcome:out,registry:out.registry||S.get().registry});if(['ACCEPTED','DELIVERED'].includes(pkg.status))toast('Server acknowledged the simulated package. No external system was contacted.','success');else toast(`Package state: ${pkg.status}`,'warning')}catch(e){S.set(st=>({busy:false,executionPackage:{...st.executionPackage,status:'FAILED'}}));toast(`Not sent: ${e.message}`,'error')}}
  async function settleCurrent(extra={}){const st=S.get();if(!st.selectedChangeIds.length){toast('Settlement requires explicit selected changes.','warning');return}try{S.set({busy:true,operation:'SETTLEMENT'});const out=await API.adapter().settle(st.activeRun.run_id,{run_id:st.activeRun.run_id,candidate_state_id:st.transition.candidate_state_id,prior_state_id:st.transition.prior_state_id,as_of_state_id:st.context.as_of_state_id,as_of_date:st.context.as_of_date,effective_date:new Date().toISOString().slice(0,10),selected_change_ids:st.selectedChangeIds,human_stop_ids:(st.transition.human_stops||[]).map(x=>x.stop_id),authority_record_ids:st.authorityRecord?[st.authorityRecord.authority_record_id]:[],execution_package_ids:st.executionPackage?[st.executionPackage.execution_package_id]:[],actor_id:actor().actor_id,allow_partial_settlement:true,idempotency_key:`SETTLE-${st.activeRun.run_id}-${[...st.selectedChangeIds].sort().join('-')}`,...extra});const nextProjection=out.projection?.projection||out.projection||st.projection;S.set({busy:false,settlement:out,registry:out.registry||st.registry,context:out.context||st.context,projection:nextProjection,view:'settled'});status('Canonical Current state returned by the server');syncRoute()}catch(e){S.set({busy:false,lastError:errorShape(e)});toast(`Settlement rejected: ${e.message}`,'error')}}
