...
IN_AREA_E   = "IN_AREA"
BEARS_ON    = "BEARS_ON"
# Semantic (V1) — TRAVERSAL_RELS in transition_engine: SUPPORTS, CONTRADICTS,
# DERIVES_FROM, DRIVES, CONDITIONS.  REFINES/TRACKS/SUPERSEDES are informational.
...
class DealGraph:
    ...
    def load_from_claims_graph(self, claims: list[dict], graph_dict: dict) -> None:
        ...
        # Add V2 nodes (model_node, case_position, support_route, artifact, decision, stubs)
        for node in graph_dict.get("nodes", []):
            nid = node.get("id", "")
            ntype = node.get("type", "")
            if ntype in (MODEL_NODE, CASE_POSITION, SUPPORT_ROUTE,
                         ARTIFACT, DECISION, RULE_SWITCH, SOLVER_CFG,
                         MODEL_CONTROL, POLICY, ROLE, INST_STATE):
                if nid and nid not in self.G:
                    self.G.add_node(nid, **{k: v for k, v in node.items() if k != "id"})

        for edge in graph_dict.get("edges", []):
            rel = edge.get("rel", "")
            src = _remap(edge.get("source", ""))
            tgt = _remap(edge.get("target", ""))
            if rel in SEMANTIC_RELS:
                ...
                self.add_semantic_edge(src, tgt, rel, **attrs)
            elif rel in V2_STRUCTURAL_RELS:
                if src in self.G and tgt in self.G and not self.G.has_edge(src, tgt):
                    self.G.add_edge(src, tgt, rel=rel,
                                    canonical=edge.get("canonical", True))

    # ── Public: analytics ────────────────────────────────────────────────────

    def pagerank(self, weight: str | None = None) -> dict[str, float]:
        """PageRank — pure Python power iteration (no numpy/scipy dependency)."""
        G = self.G
        nodes = list(G.nodes())
        N = len(nodes)
        if N == 0:
        ...
        for _ in range(100):
            ...
            for n in nodes:
                ...
                for pred in G.predecessors(n):
                    od = G.out_degree(pred)
                    if od > 0:
                        r += d * rank[pred] / od
                new_rank[n] = r
            if sum(abs(new_rank[n] - rank[n]) for n in nodes) < 1e-6 * N:
                return new_rank
            rank = new_rank
        return rank

    def betweenness(self, normalized: bool = True) -> dict[str, float]:
        """Betweenness centrality (networkx pure Python — no scipy needed)."""
    ...
    def to_vis_json(self, show_types: set[str] | None = None) -> dict:
        """
        Convert to the {nodes, edges} format expected by the graph renderer.
        show_types: which node types to include (default: V1 + V2 structural types).
        """
        if show_types is None:
            show_types = {
                CLAIM, ENTITY, METRIC, PERIOD, AREA, QUESTION, SOURCE,
                MODEL_NODE, CASE_POSITION, SUPPORT_ROUTE,
                ARTIFACT, DECISION,
            }
        ...
        for nid, data in self.G.nodes(data=True):
            ...
            nodes.append({
                ...
                "binding_direction": data.get("binding_direction", ""),
                "note":            data.get("note", ""),
            })
