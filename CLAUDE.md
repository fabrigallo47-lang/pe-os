...
**Dynamics: `backend/dynamics/`** — production runtime, adapters, schemas and policy fixtures are embedded in the backend; the API computes Candidate output directly and settlement persists replay state. `make dynamics-test` runs 45 runtime, Financial Gold, service and V20 integration tests.

**Verification: `make verify`** (`tools/verify_all.py`) — regression · V7 acceptance · V7 e2e · cascade · grounding gate · embedded dynamics · optional independent bundle validator. Pass `--reference-kit PATH` only to add the separately distributed structural validator.
