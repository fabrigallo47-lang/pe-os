...
import { actorById, composeLens, formatCount, humanState } from '../app/selectors';
...
function LensContents({ compact, onClose }: { compact: boolean; onClose: () => void }) {
  ...
  const evidenceSentence = vm.supportCount
    ? `${formatCount(vm.supportCount, 'current support')}${vm.independentCount ? ` · ${vm.independentCount} independent` : ' · no independent evidence'}`
    : 'No mapped support';
  const inspect = (id: string) => { if (!inspecting) void setActiveObject(id); };
  ...
  return <>
    ...
    <LensSection title="Why we believe this">
      ...
      {vm.supportRefs.slice(0, compact ? 2 : 3).map(item => <button key={item.id} disabled={inspecting} className="p-related-link" onClick={() => inspect(item.id)}>{item.label}</button>)}
    </LensSection>
    ...
    <LensSection title="Why it matters">
      {vm.dependents.length ? vm.dependents.slice(0, 4).map(item => <button key={item.id} disabled={inspecting} className="p-related-link" onClick={() => inspect(item.id)}>{item.label}</button>) : <p>No mapped downstream consequence.</p>}
    </LensSection>
