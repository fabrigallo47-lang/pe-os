# Product Requirements — PE Buyout Operating System

## Requirement semantics

- **MUST** identifies structural behavior required for control integrity.
- **SHOULD** identifies behavior expected in a complete product but configurable in scope or sequencing.
...
- A requirement is not satisfied by a document upload alone when the specification requires a canonical object, explicit status, relationship, decision, or authority record.
- Configurable policy may change thresholds, labels, role composition, methods, templates, or cadence; it may not disable identity, lineage, explicit authority, immutable history, or controlled transition behavior.
- Provisional analysis must remain visibly provisional. A waiver may authorize controlled use but cannot relabel provisional evidence or analysis as final.
- Machine assistance may propose, extract, calculate, reconcile, and alert. Human judgment and authority remain attributed where the relevant contract requires them.
