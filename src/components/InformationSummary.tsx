...
export function InformationSummary({ objectId }: { objectId: string }) {
  ...
  const body = claim?.normalizedStatement ?? human?.text ?? reading?.text ?? finding?.proposition ?? assumption?.statementOrValue ?? risk?.mechanism ?? decision?.rationale ?? (outcome?.observedValueOrState != null ? String(outcome.observedValueOrState) : undefined);
  ...
  const rows = [
    ['Type', kind ? humanState(kind) : human ? 'Personal view' : reading ? 'PANTA synthesis' : finding ? 'Proposed finding' : assumption ? 'Assumption' : decision ? 'Recorded decision' : undefined],
    ...
    ['Recorded', human?.recordedAt ?? decision?.recordedAt], ['Computed', reading?.computedAt],
